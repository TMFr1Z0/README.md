...
На локальный ПК установлены: VS Code с расширением Remote-SSH, MobaXTerm (основной SSH-клиент).
...
Подключение к серверу: ssh -p 2233 cxuser@89.208.175.173
Смена локали:
...
## 11. Установка pgAdmin4
    ...
    sudo docker run -d --name pgadmin4 -p 5050:80 -e PGADMIN_DEFAULT_EMAIL=admin@example.com -e PGADMIN_DEFAULT_PASSWORD=admin dpage/pgadmin4
...
Установлено расширение Remote-SSH. Конфиг ~/.ssh/config:
